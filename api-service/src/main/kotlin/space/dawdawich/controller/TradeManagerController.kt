...
class TradeManagerController(private val tradeMangerService: TradeManagerService) {
    ...
    @ResponseStatus(HttpStatus.OK)
    fun changeManagerStatus(user: Authentication, @PathVariable managerId: String, @RequestBody status: Boolean) = tradeMangerService.updateTradeManagerStatus(managerId, user.name, status)
