...
class PriceTickerListenerFactoryService(private val listenerContainer: ConcurrentKafkaListenerContainerFactory<String, String>,
    ...
    fun getPriceListener(symbol: String, isTest: Boolean): ConcurrentMessageListenerContainer<String, String> =
        listenerContainer.createContainer(TopicPartitionOffset(if (isTest) BYBIT_TEST_TICKER_TOPIC else BYBIT_TICKER_TOPIC, symbolsWithPartition[symbol] ?: throw Exception("Partition for symbol '$symbol' did not exist"), TopicPartitionOffset.SeekPosition.END))
}
